...
cast send ${ZKSYNC_REBASE_TOKEN_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft "grantAccessToMintAndBurnToken(address)" ${ZKSYNC_POOL_ADDRESS}
...
output=$(forge script ./script/Deploy.s.sol:DeployTokenAndPool --rpc-url ${SEPOLIA_RPC_URL} --account updraft --broadcast)
...
VAULT_ADDRESS=$(forge script ./script/Deploy.s.sol:DeployVault --rpc-url ${SEPOLIA_RPC_URL} --account updraft --broadcast --sig "run(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} | grep 'vault: contract Vault' | awk '{print $NF}')
